...
- (BOOL)jq_createTable:(NSString *)tableName dicOrModel:(id)parameters excludeName:(NSArray *)nameArr
{
    ...
    bool has = [fieldStr hasSuffix:@")"];
    
    NSMutableString *mutableString = [NSMutableString stringWithString:fieldStr];
    if (!has) {
        if ([mutableString hasSuffix:@","]) {
            [mutableString deleteCharactersInRange:NSMakeRange(fieldStr.length - 1, 1)];
        }
        [mutableString appendString:@")"];
    }
    
    BOOL creatFlag;
    creatFlag = [_db executeUpdate:mutableString];
