...
- (void)uploadVideoFilePath:(NSString *)filePath
    ...
    [putTask continueWithBlock:^id(OSSTask *task) {
        ...
        if (!task.error) {
            ...
            successFunc([NSString stringWithFormat:@"%@%@.%@/%@", self->subEndPointPre, self->bucket, self->subEndPointEnd, uploadPath]);
        } else {
            // 清除沙盒缓存
            [weakSelf cleanUpCache];
...
- (void)uploadImages:(NSArray<UIImage *> *)images objectPrePath:(NSString *)objectPre complete:(void(^)(NSArray<NSString *> *names, UploadImageState state))complete {
    ...
    [XNHUD showLoadingWithTitle:@"图片上传中..."];
    ...
    for (UIImage *image in images) {
        if (image) {
            NSBlockOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
                ...
                NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
                // 设置时间格式 img-880x568
                formatter.dateFormat = @"yyyyMMddHHmmss_SSS";
                NSString *name = [formatter stringFromDate:[NSDate date]];
                NSString *imageName = [NSString stringWithFormat:@"%@/%@-%.0fx%0.f.jpg", objectPre, name, image.size.width, image.size.height];
                ...
                if (!putTask.error) {
                    NSLog(@"upload object success!");
                    [XNHUD dismiss];
                    [XNHUD showSuccessWithTitle:@"上传成功"];
                } else {
                    [XNHUD dismiss];
                    [XNHUD showErrorWithTitle:@"上传失败"];
