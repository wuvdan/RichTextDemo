...
//        NSLog(@"---%@:%@", NSStringFromRange(range), selectString);
        ...
        if ([selectString isEqualToString:ImagePlaceholderTag]) {
            ...
            [html appendFormat:@"<img src=[tempImage '%ld'] />", (long)idx];
            idx ++;
//            if (attachment.image) {
...
//                [html appendFormat:@"<img src='%@' />", imageName];
//
////                NSString *imageName = [[attachment.fileWrapper.preferredFilename stringByDeletingPathExtension] stringByDeletingPathExtension];
...
//            }
            ...
            [html appendFormat:@"<span style=\"color:%@; font-size:%.0fpx; font-family:%@;\">%@</span>", textColor, fontSize, font.familyName, selectString];
